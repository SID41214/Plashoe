...
const Women = () => {
  ...
  const filterwomen = allproducts.filter((item) => item.category === "women");
  const { searchTerm, islogin } = useContext(MyContext);
  const [duplicateData, setDuplicateData] = useState(allproducts);

  // useEffect(() => {
  //   function filterShoeData() {
  //     const result = filterwomen.filter((item) =>
